...
- A usage-only push uploads the updated day file and refreshes last-seen/sidecar, while preserving activity counts and the cursor. It skips automatic fleet GC; obsolete blobs await a content-changing push or `mm gc`. Autopush stays change-gated and never warms; previews never capture, and `mm push --dry-run` now lists host-usage capture among what it does not preview.
...
- JSONL appends separate torn suffixes from the next valid row and refuse non-regular targets before touching their permissions; rollback remains best effort. Init Git rows carry their own origin and updated renderers exclude them from push counts while preserving legacy absent/unknown-origin behavior. These close a counting class before it scales; the measured history had one refresh miscount and no init-origin rows.
...
- Pull checks case collisions on the nearest existing ancestor when the destination root is missing, preventing case-only peer paths from overwriting each other on macOS. A symlinked source root's case-sensitivity check now probes the link's destination, not the volume the link itself lives on.
...
- **`conflictdiff.merge_has_line_structure(local_lines, remote_lines)`** — pure leaf predicate, False when either side has `<= 1` line. Both prompt sites AND it into `merge_available`, so `(m)erge` disappears from the prompt for a minified one-line file. `lcs_merge` builds a synthetic ancestor from `LCS(local, remote)`; with one line there is nothing to align and the only possible output is a single `<<<<<<<` region wrapping both versions whole — never a valid file of that type, and strictly worse than `(l)ocal` / `(r)emote` because it adds a mandatory editor round-trip to reach a state one keystroke already gave you. Order-independent (so `mm resolve`'s inversion-mode argument swap can't change the verdict), routes through the existing binary-content gate (so a typed `m` degrades to keep-both with no new apply branch), and suppresses the empty-side case too (`lcs_merge(b"", remote)` returns remote as a "clean" merge, which is `(r)emote` in disguise). Deliberately narrow: `<= 1` line, NOT "no shared lines" — two multi-line documents with zero overlap still get `(m)erge`, and the v0.12.8 never-default-to-`(m)` rule already covers that footgun.

- **`config._GENERATED_HOST_SKILL_GLOBS`** — the five gstack-extend skills that `setup --host auto` RENDERS per host from one shared source (`roadmap`, `full-review`, `test-plan`, `pair-review`, `review-apparatus`; the two `gstack-extend-*` skills were already covered by `skills/gstack-*`). Shared by the codex and opencode entries and splatted into a fresh list at each use site, because `get_default_source` hands these lists to callers that mutate them.
...
- **Four new default `exclude_patterns`, retiring 43 of 88 recorded conflicts.** `projects/*/decisions.active.json` (gstack, 18 conflicts + 66 mtime-skips) and `projects/*/brain-cache/*` (gstack, 7 + 40); `skills/.system/*` (codex, 16); `_GENERATED_HOST_SKILL_GLOBS` (codex + opencode, 2). Counted by matching each glob against the 88 records, not by eye: an earlier draft said 44 and credited `.system` with 17, double-counting the `skills/roadmap/SKILL.md` conflict that belongs to the generated-renders group. Existing configs opt in with `mm migrate-config` — autopull/autopush still never auto-mutate config.
  ...
  `skills/.system/*` is Codex's own bundled payload (imagegen, plugin-creator, skill-creator, …), reinstalled by Codex and version-stamped in `.codex-system-skills.marker`. Two Macs on different Codex builds diverge by construction — this produced 17 conflicts in a single pull, not one of them a file either machine wrote. Codex-only; OpenCode has no `skills/.system`.

- **The sync-log conflict header no longer describes the pre-inversion direction.** `.mind-meld-log.md` said `## Conflicts (remote differed, local preserved as .sync-conflict-*)`, which reads as "your local edits were moved aside" — the v0.9.2 inversion made the sidecar hold REMOTE bytes with local untouched at canonical. Now `remote saved as .sync-conflict-*`. Wrong since v0.9.2 and purely cosmetic, but it told the user the opposite of what happened at exactly the moment they were deciding what to do about it.

### Not done, deliberately

- **No JSON/YAML merge strategy in `merge.py`.** `decisions.active.json` is a JSON array of `id`-keyed records, so a union-by-`id` merge looks obviously right — and is wrong. `computeActive()` deliberately OMITS superseded decisions, so a union would resurrect every decision any machine had retired. Exclusion is the only resolution that converges. `test_gstack_derived_cache_globs_match_observed_paths` pins that `decisions.jsonl` and `decisions.archive.jsonl` are NOT caught by the new globs.

- **`pair-review/` and `full-review/` stay in sync scope** (31 of 88 conflicts: `session.yaml`, `deploy.md`, `report.md`, `parked-bugs.md`). These are live per-machine session state, but pair-review advertises cross-machine resume as a feature, so a blanket exclusion would remove capability rather than noise. Needs a device-scoping design upstream; `session.yaml` is the narrow fallback.

- **The Phase 2 auto-resolver is cancelled, on evidence.** The v0.12.4 conflict-decision collector was removed in Track 16A on the stated grounds that it "collected zero decisions" and its log "never existed on the fleet." Both claims were false — `~/.config/mind-meld/conflict-decisions.jsonl` holds 9 decisions. Reading them kills the feature more thoroughly than the false premise did: `choice: remote` in 9/9 is **tautological**, because mtime-skip means the conflict path only fires when remote is newer-or-equal, so `newer_side` can only ever be `remote`. There is no preference to learn, and 8/9 records were `similarity: 0.0` single-line JSON — the derived-cache noise this release excludes. After these globs, residual hand-authored conflicts run ~5 per four months, which is precisely the workload that should stay human. Do not rebuild the collector; a fifth attempt needs a mechanism whose output isn't fixed by construction.

- **Detection of generated skills is by name, not by marker.** gstack-extend drops a `.extend-root` file in each rendered dir — the robust signal — but `exclude_patterns` are fnmatch globs against a relative path and cannot express "skip the directory CONTAINING this file." A new gstack-extend skill needs a new glob until a marker-aware walker skip lands.
...
- A machine whose git walks all held on `discovery: partial` no longer disappears from the card. The gap loop keys on OBSERVATIONS, not on coverage: a held capture still paints nothing, but it proves the device pushed, so a held-only device reports its whole window uncovered and a run of held pushes after the last good capture is no longer clipped away. Silence there read as "no known coverage issue" on exactly the machines that needed `mm recapture`. `discovery: empty` is deliberately excluded (new `events.DISCOVERY_EMPTY`): a prober that ran and found zero git roots is a fact about the machine, not a loss, and a repo-less Mac must never be told to recapture — it already gets the zero-repo push note.
