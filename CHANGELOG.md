...
Track 1B: manifest dead-code cleanup + v1-holdover removal. Drops vestigial
back-compat aliases and the redundant top-level `files` key from v2 manifests.
Zero behavior change for current users; tightens the data model so future
contributors have one shape to reason about instead of two.
...
- `SPEC.md` updated: manifest schema section no longer describes the dead
  ...
  auto-promoted on load and how pre-v0.8.1 v2 manifests are auto-scrubbed.
