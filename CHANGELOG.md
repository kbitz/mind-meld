...
**Test coverage.** `tests/test_conflict_copy.py`: `TestNeverDefaultToMerge` (default key is `s` at both sites on a clean merge; `(m)erge` still works when typed), `TestPromoteHelpers` (per-mode naming, none-peer fallback, collision → `-<4hex>`, `os.link` no-clobber, happy path), `TestResolvePromote` (post-inversion + pre-inversion promote, link `OSError` → failed count, `include_files` warning vs `include_dirs` no-warning, no-base promote unchanged). `tests/test_conflictdiff.py`: `render_prompt` `promote_available` line. 1575 tests pass.
