...
`conflictlog.py` (TEMPORARY, v0.12.x — CONFLICT-TELEMETRY) — disposable labeled-dataset collector for the deferred Phase 2 auto-resolver. `append_decision(**fields) -> bool` (own flock write, returns real status, NEVER raises) appends to `~/.config/mind-meld/conflict-decisions.jsonl` (local-only, not synced). Hooked resolve-site-only into `_resolve_interactive_loop` (+ helpers `_conflict_feature_dict` / `_conflict_rel_path`, shared feature extraction using `merge.similarity_ratio` for classifier parity) plus a hidden `mm conflict-log-backfill` command (quarantined `site=backfill` rows, under the mm lock). NO autopull/hot-path change; NOT in `_apply_incoming_file`. Every call site carries a grep-able `# CONFLICT-TELEMETRY (temporary)` sentinel. Rip out per the `[plan-eng-review]` TODO once Phase 2 thresholds validate; design at `~/.gstack/projects/kbitz-mind-meld/kb-kbitz-conflict-resolution-log-design-20260730.md`.
