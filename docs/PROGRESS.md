...
- **Group 5 (Conflict UX & first-pull polish):** in flight, nearly done.
  **Track 5A ✅ v0.8.15** (P0 autopull silent-mode contract regression +
  ...
  from "conflict inversion + real-merge" via /plan-ceo-review after kb-mbp
  first-pull data showed 24/25 divergent files were per-machine artifacts).
  **Track 5E ✅ v0.9.2 BREAKING** (inverted `_apply_conflict` so canonical
  ...
  pick up). **Track 5D (last remaining):** `_find_conflict_files`
  double-count dedup + `_save_and_register` crash-safety beyond Python
  exceptions (SIGKILL/OOM/power-loss window). See ROADMAP.md.
