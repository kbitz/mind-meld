...
### Group 52: Terminal-control postcondition ✓ Shipped (v0.14.6)

- Track 52A — _shipped at bb39230 (v0.14.6, #169): `strip_terminal_escapes`, `safe_str` and `safe_text` strip the known CSI / OSC / DCS / C1 grammars and then delete every leftover ESC (`U+001B`) and C1 (`U+0080`–`U+009F`) code point, so a nested sequence cannot reassemble into a fresh introducer; line feeds and tabs still pass for diff bodies, and the guarantee is ESC/C1, not all-control. Grammar removal lives in a private `_strip_terminal_grammar` shared by both policies; `safe_terminal_str` keeps its visible notation so a diagnostic never silently loses a byte. The three named plain-stderr sites (retention.py's token-cache GC notice, token_usage.py's oversize-line and unknown-model notices) moved to `safe_terminal_str`; the RIS (`ESC c`) comment that claimed single-byte coverage was corrected. init-devices.md and the module map state the postcondition. Shipped at 624 insertions against the card's ~80 LOC, widening onto AGENTS.md and `tests/test_conflictdiff.py`. Left as recorded follow-ups: the other nine plain-stderr `safe_str` sites and any broader C0 policy (roadmap-future.md)._

### Group 53: Pull isolation ✓ Shipped (v0.14.7)

- Track 53A — _shipped at fe21a57 (v0.14.7, #170): `_download_and_apply` catches exactly `(OSError, MindMeldError)` per file; `Abort`, `SystemExit`, `KeyboardInterrupt` and programming errors still propagate. A new `_ApplyReporter` records each file's outcome at the moment of successful atomic write, before any fallible follow-up, so a crash in stale-copy cleanup, a notice or the mtime restore can no longer erase a completed publication from history or the sync log. Mkdir failures name the blocking collider and its peer and register every created ancestor for fsync; an `OSError` from the final `os.close` inside directory fsync no longer replaces the original exception; the `--conflict-mode fail` preflight and the mtime gate tolerate malformed peer mtimes; `load_manifest` rejects a non-string `sha256` and coerces a non-string `mtime` to `None`. Interactive `mm pull` exits 0 for contained per-file failures, matching autopull, and both print `Pull incomplete: N file(s) failed` plus one `mm: warning:` line per file through the same stderr formatter; the `degraded` breadcrumb covers per-file failures. Shipped at 1,790 insertions against the card's ~150 LOC (cli.py alone +498/-211), widening onto README.md, AGENTS.md, `errors.py`, `manifest.py`, `tests/test_conflict_copy.py` and `tests/test_manifest.py`. Two accepted residual interrupt windows are recorded in sync.md and conflicts.md. The interactive exit-code question was put to the user at the gate and the exit-0 contract kept (roadmap-future.md)._

### Group 54: Honest Codex diagnostics ✓ Shipped (v0.14.8)

- Track 54A — _shipped at 735ed02 (v0.14.8, #171): Codex and Grok share one standing-blocker policy — `unsupported` survives transient failures, a completed read clears it, migrated undated blockers are dated once, and post-lock deadline expiry records the blocker without scanning. Both readers keep `last_reason` and `last_reason_since`; `mm status` reports any standing reason before inventory-based remedies (Codex advice source-gated) and `mm diag` labels cache inventory separately and says `none` for a readable healthy cache. Cache-write errors emit one notice while retaining the scan result; repeated unchanged failures avoid rewriting the cache. `_model_id`, the test-only `_terminal_from_record` / `_Terminal` path and their tests were removed; the production `_TurnState` cumulative union is unchanged and pinned through cold, warm and appended reads. README gained host-usage troubleshooting, diagnostic keys, source-disable consequences and a rollback command. Shipped at 1,077 insertions against the card's ~120 LOC + ~60 deleted, widening onto AGENTS.md, `events_tail.py`, `tests/test_host_usage_snapshot.py`, `tests/test_init_events_backfill.py` and `tests/test_source_toggle.py`. Deferred at the gate by user decision: the automatic no-op-push re-read of a standing blocker (T3-B, roadmap-future.md); the shipped wording says "the next push that uploads a change retries" instead._

### Group 55: Git capture integrity and cleanup ✓ Shipped (v0.14.9)

- Track 55A — _shipped at d85e504 (v0.14.9, #172): all four git subprocesses (history, origin, and both identity reads) run under one scrubbed environment from the new leaf `gitenv.py` — repository-redirection and command-scoped configuration variables can no longer substitute another repository's history, origin or configured identity; global and repository config remain supported; a drift check against `git rev-parse --local-env-vars` and an AST gate protect future call sites; Git's message locale is pinned to C so a localized no-commits message is not classified `git_error`, and an undecodable byte skips an identifier instead of aborting the walk. The identity cache refreshes once after an upgrade when discovery completes; an incomplete attempt keeps the prior emails in a stale cache and retries. `_last_mm_push_ts` and `_run_events_recapture` were deleted and their invariant facts re-pointed to `_iter_mm_push_objs` and `resolve_push_cursor`; `mm recapture --dry-run` names skipped repositories and benign empty repos, and partial recapture distinguishes Git failures from budget aborts. Shipped at 1,020 insertions against the card's ~40 LOC + ~50 deleted, widening onto README.md, `cli.py`, `errors.py`, `identity.py`, the new `gitenv.py` / `tests/test_gitenv.py`, `tests/test_identity.py`, `tests/test_docs_routing.py` and `tests/test_silent_failure_contract.py` — the /autoplan settled plan (2026-09-10) added the gitenv leaf, the AST gate and the identity-cache version rule to the card. Left deferred: a `mm diag` line listing the ignored variables (E6, roadmap-future.md)._

### Group 56: Dry-run honesty ✓ Shipped (v0.14.10)

- Track 56A — _shipped at dbc926e (v0.14.10, #173): `mm push --dry-run` changes nothing except the local lock file. It no longer writes a config migration, persists a missing crypto fingerprint, records a pull-history row, or creates the mm-events directory; it prints what a real push would still set up, and refuses (exit 1) with a remedy when the mm-events directory is missing but this Mac previously published files from it, instead of silently proving an empty deletion. New seams: `_get_config(read_only=)`, `_init_crypto_session(read_only=, pending=)`, `resolve_sources(bootstrap=False)` with `SourceResolution.would_create`, `crypto.fetch_crypto_init(repair=False)` with `CryptoInitRepairPlan`, and a nudge gate. `mm recapture --dry-run`'s completion message was corrected to "no commits written or uploaded" — its own writes were not fixed. Help text and README document what stays untouched and the exit codes (0 / 1 / 2). Shipped at 915 insertions against the card's ~110 LOC, widening onto `crypto.py`, `tests/test_crypto.py`, SPEC.md, AGENTS.md, four invariant docs (`auto-upgrade.md`, `events-retro.md`, `init-devices.md`, `sync.md`), and one-line mock-signature fallout in `tests/test_pull_helpers.py`, `tests/test_safe_str.py`, `tests/test_skill_link.py` and `tests/test_track_30a.py`; `upgrade.py` and `tests/test_upgrade.py` were declared but untouched (the nudge was gated at the call site). The /autoplan review (2026-09-10) filed four follow-ups: E4, E7, E3b and 56B are Tracks 59A, 60A and 62A in the 2026-09-14 plan._

### Group 57: Grok pricing ✓ Shipped (v0.14.11)

- Track 57A — _shipped at a7d9bca (v0.14.11, #174), re-scoped by /autoplan (2026-09-11, decision D1 = A): Grok had never reached the wire — the reader repair in v0.14.1 counted tokens locally, but the autopush host budget let Codex's warm read (~186 ms of 250) starve Grok's — so delivery and pricing shipped together. Later readers get a 50 ms grace floor (`HOST_READER_GRACE_MS`); attended captures warm every deadline-dropped warmable reader before retrying each successful warm on its own budget. `grok-4.6-build` prices by exact alias onto the verified grok-4.6 card (xAI and OpenAI provenance 2026-09-10); `grok-build-0.1` stays unpriced; Grok always renders `>=`, with a model-scoped at-most figure only when coverage allows it, and no billed-tick decoding (the 303-turn `costUsdTicks` census fits the rate shape exactly but sits at 0.17x list under xAI's documented unit, so it is recorded on the Future bullet and not used). `gpt-6-astra` is priced. `mm diag` reports `grok ledgers cached: N of M` without fabricating zeros; README documents `mm recapture 1d` as the attended refresh bridge. Shipped at 1,169 insertions against the card's ~100 LOC, widening onto `cli.py`, `events_tail.py`, `host_usage.py`, `lockedjson.py`, SKILL.md, AGENTS.md and seven test modules beyond the two declared. The census ran on Grok 1.0.25 but the contract pin still says 1.0.13 — Track 58A (2026-09-14 plan) moves it._

Phase 3 (Retro fidelity) continues: Group 57 closes here. Group 58 in the 2026-09-14 plan carries the rest — the 2026-09-06 plan's 58A (usage presentation) absorbed the Anthropic rate refresh and the Grok census re-pin on 2026-09-14 by user decision (D1 = A), so one PR closes the Phase.

### Group 58: Verified rates and usage presentation ✓ Shipped (v0.14.12)

- Track 58A — _shipped at dfc7a09 (v0.14.12, #176), settled by /autoplan on 2026-09-14 (D1 = B: disclose fast mode, defer the detector): the retro shows input, cache-write, cache-read and output tokens together with per-model dollars; per-machine model subtotals sit in the economics section, scope lines name the source logs, observation dates and coverage, and Claude and host-log figures are never added together. Anthropic cards were re-read and verified on 2026-09-14 — Sonnet 5 at $2/$10, the Sonnet 4.x ids kept at $3/$15, Fable 5.1 and Mythos 5.1 cache reads at $0.25 — family-extrapolated rates are named in Notes, and floors price cache writes at the minimum rate so `>=` is arithmetically a floor. One window-eligibility rule now covers agent activity and economics (a stale snapshot no longer contributes on the first UTC day), a present empty token map is an observed zero, and one machine-label format serves activity, economics and model subtotals. Three card premises were corrected at review: the Grok census was re-run and pinned to 1.0.30, the installed host, not the carded 1.0.25 (zero drift, sanitized fixture and reproduction recipe added); `claude-opus-4-0` is Anthropic's published alias and was kept, with `claude-opus-4` added because the dated id priced 3x low; and the carded `_read_events` deletion landed with the renderer work. Fast-mode turns stay priced at standard rates with the 2x limitation disclosed. Shipped at 1,753 insertions against the card's ~275 LOC across 26 files, widening onto `docs/designs/grok-build-usage-reader.md`, five `tests/fixtures/retro_usage/` goldens, the new `tests/test_retro_usage_presentation.py`, `tests/test_docs_routing.py`, `tests/test_integration.py` and two other test modules. Left deferred: the fast-mode detector and shared-prefix device labels (roadmap-future.md)._

Phase 3 (Retro fidelity) ✓ complete at v0.14.12: Groups 57 and 58.

### Group 59: mm-events root ownership ✓ Shipped (v0.14.13)

- Track 59A — _shipped at e7eb301 (v0.14.13, #177), settled by /autoplan on 2026-09-15 with Track 60A folded into the same PR by user decision: source resolution defaults to read-only, and only the writers create the default mm-events root at 0700 — init, real push and autopush, recapture, and a pull applying mm-events files (skill installation creates the shared root at 0700 too). **The card's second task was reversed at the gate (D2 = A):** a missing default event file means it was deleted, so push publishes the deletion with no loss check and no refusal, and v0.14.10's preview refusal was removed so preview matches the real push, including when only `events/` is gone. The reversal rested on a read-only check that found 0 of 54 published event files missing and all 61 mm-events tombstones explained by gc retention. No automatic restoration runs; README documents restoring before the next push. A missing custom mm-events folder is warned about and skipped for that push with no new tombstones and a degraded autopush breadcrumb; recapture refuses before writing rows and pull skips the source with a warning. Pull checks case collisions on the nearest existing ancestor when the destination root is missing, and a symlinked root's case-sensitivity probe follows the link. Shipped with 60A at 1,793 insertions across 29 files against two cards of ~120 LOC each, widening onto `retention.py`, `skill_link.py`, `storage/local.py`, `aggregator.py`, `tests/conftest.py` and six test modules beyond those declared._

### Group 60: Inspection without repair ✓ Shipped (v0.14.13)

- Track 60A — _shipped at e7eb301 (v0.14.13, #177), inside Track 59A's PR: `fetch_crypto_init` is a pure fetch, and shared crypto-init repair runs only after the drift check and passphrase verification, for mutating commands only. `mm status`, `mm diag`, `mm diff` and the pull, gc and recapture previews open read-only crypto sessions, report pending repair as counts, and never persist a missing fingerprint. Verified repair deletes only byte-identical conflict copies and durably preserves distinct bytes (same-salt copies with different parameters or keychecks included) as `mm-crypto-init.preserved-*`; symlinked conflict names are treated as unreadable; the canonical rewrite is fsynced before any copy is touched; a winner that changes mid-command aborts before publication or init registration; and a copy is deleted only while the canonical still holds its exact bytes at the last possible moment, so two concurrent repairs cannot erase the only verified copy. `mm status` reads the upgrade cache with no HTTP request and no write, reports unknown cache states and stale check age, and leaves pending upgrade transitions for the next mutating command. Filed as follow-ups and drained 2026-09-17: the `_open_crypto_session` helper extraction (roadmap-future.md); the redundant re-read cleanup was killed._

### Group 61: Attended host-usage refresh ✓ Shipped (v0.14.14)

- Track 61A — _shipped at 5079b3f (v0.14.14, #178): `mm push --capture-usage` refreshes host usage on an attended push even on a converged Mac. It shares one host-only capture helper (`_capture_host_snapshot`) with the tail and the init backfill, performs one bounded sweep with a single warm/retry, writes no `mm-push` row, and refuses incompatible flags and missing source or consent before the Keychain. Success requires the requested row's file revision in the accepted manifest — append errors, short writes (`flock_append_jsonl(strict=True)`), source exclusions and size limits cannot produce a false success — and post-publication maintenance failures are reported separately. Exit codes: 0 published, 4 capture missing or unpublished, 1 stopped before acceptance, 2 incompatible with `--dry-run`. `mm status` and top-level `diag.host_publication` separate the last recorded capture, per-reader coverage, manifest-backed publication and the unknown latest attempt, using the fleet's own acceptance and ordering (`latest_event_rows`, `local_host_capture_candidate`); fleet retros carry one aggregated staleness note. `mm enable-source` and `mm reconfigure-sources` materialize default sources from an explicitly empty list, and `mm recapture` points cold or blocked host caches at the flag. The /ship review found the per-reader outcome label keyed on whole-sweep state; a first fix keyed on family names was itself wrong (`host_family()` classifies by model-id prefix) and a later adversarial pass replaced it with the reader-identity-scoped `HostUsageCapture.empty`. Shipped at 2,108 insertions across 28 files against a card re-sized at review to ~1,850 diff lines. Follow-ups drained 2026-09-17: the same whole-row emptiness on the status/diag read path, the claimed digest race and three scan costs (the "Reader-scoped publication evidence" Track), and git/session rows still written under the flag plus the exit-4 wording (the "A usage refresh is not a push" Track), both in the 2026-09-17 plan._

### Group 62: Write-free previews ✓ Shipped (v0.14.15)

- Track 62A — _shipped at 66b5293 (v0.14.15, #179), settled by /autoplan with the user overriding the recommended split so the multi-peer predictor shipped in the same PR: pull, gc and recapture previews change nothing except the local lock file, and `migrate-config --dry-run` and `mm diff` take no lock at all. Previews skip config migration, version-transition records, upgrade checks, pre-v0.9.2 conflict-file migration and excluded-path history. Every `_get_config` and `_maybe_prompt_migration` call states `read_only=` explicitly (AST-enforced, no default), and `COMMAND_INTENTS62` classifies all 23 commands and audits each preview and inspection for in-process mutation, worker threads included; status reads seen-sources state without writes and preserves concurrent acknowledgments during seed recovery. The new leaf `pullplan.py` tracks virtual canonical state across successive peers — symlinks, unreadable files, tombstones, exclusions and file/directory collisions — so two peers changing a mergeable file no longer cause a false `--conflict-mode fail` exit 3; merges remain an upper bound and blob or decrypt failures are not previewed. Fail-mode treats FIFOs, sockets and other non-regular files as predicted failures, and `mm diff` applies local exclusions to either comparison target. Shipped at 2,019 insertions across 22 files against the card's ~250 LOC, widening onto `pullplan.py`, `seen_sources.py`, `retention.py`, SPEC.md, AGENTS.md, `init-devices.md` and five test modules beyond those declared; `resolveflow.py` was declared but untouched. Follow-ups drained 2026-09-17 to roadmap-future.md: pull-history rotation, gc and recapture parity tests, and the README walkthrough; SPEC.md's missing `mm recapture` and `mm push --capture-usage` entries ride the "A usage refresh is not a push" Track in the 2026-09-17 plan._

### Group 63: Codex read headroom ✓ Shipped (v0.14.16)

- Track 63A — _shipped at 77338bd (v0.14.16, #181), from the 2026-09-17 effective spec (UC1/UC2, TASTE-1 through TASTE-6): the Codex validator and reducer were rewritten behind a frozen v0.14.15 oracle (`tests/_host_usage_oracle.py`; 200 seeded lineages and the live corpus, byte-identical ordered totals), both readers pause cyclic GC and restore the caller's state on every exit, both host caches write compact JSON without a version bump, and `[retro] host_usage_autopush_budget_ms` / `host_usage_interactive_budget_ms` give each Mac a read-budget lever. Attended captures publish the completed warm read itself with no second bounded read; a complete-but-late read records `deadline`, its allotted time and its last complete read while still committing the cache and pruning deleted files. `mm diag` shows each reader's last complete read and age, the effective budget sources and a consent-scoped sweep estimate; `diag --json` gains `host_read_budgets`, `last_complete_ms`, `last_complete_at` and `last_deadline_allotted_ms`. Qualification on device `3a6c7dc9`, Python 3.14.7: three warm 250 ms reads at 168.26 / 166.66 / 166.90 ms before serialization over 1,066 rollouts, against 384 ms and 3 of 3 deadlines at 70abdee. Shipped at 2,465 insertions across 25 files. The implementation commit hand-edited this card against the plan's own acceptance criterion; this entry is the formal re-card (drained 2026-09-21). Follow-ups drained 2026-09-21: the deadline-allotment jitter rides Track 66A; the lineage tier, the compact-cache measurement and the `token_usage_record` tripwire went to roadmap-future.md; the read-budget predictor was killed._

### Group 64: Usage-only capture ✓ Shipped (v0.14.17)

- Track 64A — _shipped at a12096c (v0.14.17, #182), **re-scoped past the card at review:** the card kept `mm push --capture-usage` and corrected its exit-4 wording; what shipped removed the flag without an alias, and every attended `mm push` now captures consented host usage under the lock after final source resolution, converged tree included. Usage-only publication appends one host row, preserves activity counts and the Git cursor, and skips automatic fleet GC; a content-carrying push captures activity once. Exit 0 means content sync succeeded regardless of capture outcome (capture failures warn and continue, with no second reader attempt); exit 4 is recapture-only. Init Git rows carry `origin=init` and renderers exclude them from push counts; JSONL appends separate a torn prior suffix and refuse non-regular targets before chmod. SPEC.md gained `mm recapture` and the attended-capture contract, status and diag name `mm enable-source mm-events` instead of a command that would refuse, and the skill preflight pins the v0.14.17 binary floor. Shipped at 2,825 insertions across 31 files against the card's ~150 LOC. Absorbed the Future bullet "Re-read standing host-usage blockers on an interactive no-op push (T3-B)", discharged 2026-09-21._

### Group 65: Reader-scoped publication evidence ✓ Shipped (v0.14.18)

- Track 65A — _shipped at cdffc34 (v0.14.18, #183), settled by /autoplan on 2026-09-21 with a fifth task added at the gate: host snapshots carry per-reader `empty_sources` from each reader's own usage days within the retained snapshot, and verbose push, status and diag share the label `completed, no usage in snapshot`; publication verification uses one fully consumed parser pass, so configuration exclusion, an absent manifest file or accepted bytes missing the row prove `not-published` while unreadable, oversized, changed or post-acceptance-mismatched reads are `unverified` (the `unreadable-row` token is retired for `row-missing`); local host selection bounds an unreadable day file's uncertainty by the winning timestamp and allowed clock skew; and the diagnostic hash cache keys by file identity so a `resolve()` failure cannot drop a file. The added task is a private mode-0600 `last-attended-capture.json` written under the push lock (`attemptlog.py`) and rendered on status and diag as the latest attended attempt, separately from the last recorded capture; it consumed the only reader the Future bullet "Say on the wire whether a host capture was requested" had, so that bullet is discharged. A pre-landing fix opens the next UTC day for the cursor so a clock rollback cannot hide a push row. Shipped at 1,981 insertions across 20 files against the card's ~200 LOC. Its /autoplan filed the publication-evidence standing constraint now recorded at the top of ROADMAP.md._
