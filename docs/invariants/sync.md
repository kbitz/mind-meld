...
**Detection is by name AND by marker (v0.14.0, Track 47A).** gstack-extend drops a `.extend-root` file in each dir it renders. `manifest.marker_skip_globs` turns those into `rel_dir/*` globs; `walk_generic_source` merges them into `exclude_patterns`, and `cli._build_exclude_map` merges the same globs so `_filter_excluded_paths` strips them from prior/peer manifests. A marker skip must not generate deletion tombstones, exactly as adding a glob must not. The name-based `_GENERATED_HOST_SKILL_GLOBS` stay as belt-and-braces for dirs that lost their marker; a new gstack-extend skill no longer needs a new glob to drop out of sync.
