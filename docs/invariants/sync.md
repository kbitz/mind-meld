...
`mm push --dry-run` changes nothing except the local lock file, including the lock parent's creation when needed. `_get_config(read_only=True)` skips the entire transition hook, and push skips its second config load because migration cannot run. Migration emits the existing warning; only when both streams are TTYs does it explain that the preview uses current config and a real push offers `mm migrate-config` (default yes). No confirm prompt runs. Crypto repair and mm-events bootstrap are planned; fingerprint backfill stays in memory. The nudge is gated at the CLI call site. No changes to `lockfile.py`, `upgrade.py`, or `_prove_omitted_paths_absent` implement this contract.

**Missing mm-events root (PC3).** After filtering the recovered prior through selected sources, exclusions and symlink omissions, a preview with `would_create` refuses if that source has prior files. Its count uses this filtered prior, including sidecar/peer recovery, never the raw fetch. The message explains the real push's empty-root deletion, peer/backup restoration, verification of `<path>/events`, and the explicit accept-deletion path. With zero known prior files, omit only that root from the deletion proof's source list. Do not introduce an assume-empty bypass into the proof. A present root with a missing `events/` subdirectory remains a deletion preview. Real-push bootstrap semantics remain deferred to E4.

Successful previews always print pending setup and “Dry run complete. Nothing was changed except the local lock file.” plus the not-previewed scope (activity row, GC, upload re-reads). Refusals never print completion and carry the lock-qualified suffix. Exit codes remain 0 completed, 1 stopped, 2 usage error. Tests in `TestPushPreviewNoMutation56A` record filesystem/network/keyring mutation attempts and compare the entire isolated tree with pinned mtimes. This is push-only; sibling previews are deferred to 56B.
