...
**`_canonical_for_conflict` treats empty or invalid original filenames as ownerless.** An infix-at-index-0 name (``.sync-conflict-<ts>-<dev>``) reconstructs to the empty string; trailing-dot shapes can reconstruct to `.` or `..`. These name no canonical file. Return `None` so discovery records no canonical, GC preserves the sidecar, and promotion fails without mutation. Passing an empty name or `.` to `Path.with_name` would raise; accepting `..` would identify a directory. Returning the sidecar itself would make resolve/GC treat the only copy as both sides and delete it.
...
- **INVALIDATION** lives in `_ApplyReporter.record`, keyed on the outcome via the `_CANONICAL_WRITE_OUTCOMES = frozenset({"written", "merged", "merged-via-lcs", "conflicted"})` constant. At publication, or at a non-publishing return such as sidecar dedup, it pops the corresponding entry when `outcome in _CANONICAL_WRITE_OUTCOMES`. The other outcomes (`"failed"`, `"skipped"`, `"unchanged"`) leave canonical untouched and intentionally do NOT invalidate.
...
**Why centralize in `_ApplyReporter.record`, not per-branch in `_apply_incoming_file`.** ONE site keyed on the outcome enum covers every canonical-mutating path uniformly: keep-remote, inline merge, the keep-both → `_apply_conflict` sidecar, AND the `_apply_write` branch that fires when canonical vanished mid-walk (user `rm`'d the file while the blocking prompt waited, or an autopull race deleted it). Publication-time recording also voids the bump before a fallible restore/cleanup/print can raise. A per-branch approach missed `_apply_write` entirely — it would silently bump a LATER peer's REMOTE bytes as locally-authored when the file was deleted between peer A's prompt and peer B's apply (Codex adversarial HIGH 2026-05-14). It was also not success-only for keep-both — popping before the `_apply_conflict` call would drop the keep-canonical decision even when the sidecar write failed. Both holes were closed by routing through the outcome enum. The deterministic `_apply_merge` path (`should_merge` true) is ALSO covered: it returns `"merged"`, which is in `_CANONICAL_WRITE_OUTCOMES`, so the gate fires uniformly there too. Unrecorded canonical-write returns such as sidecar dedup still invalidate at the `_download_and_apply` boundary.
...
**Abort bookkeeping (Track 53A).** Abort still skips the drain. Completed publications and completed keep-local returns keep their history and sync-log entries; recorded touched parents are fsynced before recovery bookkeeping. `_ApplyReporter` invalidates pending bumps at publication, before a fallible follow-up can raise, and validates the returned word at `_download_and_apply`. A failed write leaves an earlier keep-local decision eligible; a published write/merge/sidecar voids it even if the pull then aborts. Nothing is rolled back. The publication-to-reporting and second-Ctrl-C residual windows are documented in sync.md's "Apply exception boundary" section.
...
**`(m)erge` is suppressed when either side has no line structure (v0.12.51).** `conflictdiff.merge_has_line_structure(local_lines, remote_lines)` returns False when either side has `<= 1` line, and both prompt sites AND it into `merge_available`:

```python
merge_available = merge_conflicts >= 0 and merge_has_line_structure(local_text, remote_text)
```

`lcs_merge` is line-based — it builds a synthetic ancestor from `LCS(local, remote)`. With a single line on either side there is nothing to align, so the only possible output is one `<<<<<<<` region wrapping both versions whole: never a valid file of that type, and strictly worse for the user than `(l)ocal` / `(r)emote` because it adds a mandatory editor round-trip to reach a state one keystroke already gave them. Every one of the nine decisions in the 2026-08 `conflict-decisions.jsonl` sample had exactly this shape — `merge_available: true, merge_conflicts: 1` on `local_lines: 1, remote_lines: 1` — because gstack writes `decisions.active.json` as one minified `JSON.stringify` line. (That file is excluded from sync as of v0.12.51; see `sync.md`. The suppression is the general guard, not a fix for that one path.)

Three properties this relies on, all deliberate:
* **The predicate is order-independent**, so `_resolve_interactive_loop`'s inversion-mode argument swap (`lcs_merge(cpath, canonical)` vs `lcs_merge(canonical, cpath)`) cannot change the verdict.
* **Suppression reuses the binary-content gate.** It only ever sets `merge_available` to False, so the existing "typed `m` while unavailable degrades to keep-both" contract (see the refusal note above) applies verbatim — no new apply branch, and no path where a marker-laden buffer reaches canonical.
* **A zero-line (empty) side is suppressed too.** `lcs_merge(b"", remote)` returns remote as a "clean" merge, which is `(r)emote` in disguise; the same reasoning as the `local_read_failed` guard in `_prompt_conflict_choice`.

Keep it narrow: the suppression is `<= 1` line, NOT "no shared lines." Two multi-line documents with zero overlap still get `(m)erge`, because the markered output is at least editable there, and the never-default-to-`(m)` rule above already covers that footgun. Pinned by `tests/test_conflictdiff.py::TestMergeHasLineStructure` (pure) and `tests/test_conflict_copy.py::TestSuppressMergeWithoutLineStructure` (both prompt sites, plus a `test_multiline_file_still_offers_merge` regression guard against over-broad suppression).
