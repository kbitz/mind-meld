...
**Read-only source resolution (Track 56A).** `resolve_sources` / `get_sources` default to `bootstrap=True`. Push preview passes `bootstrap=False`: a missing mm-owned root remains selected and available as an empty walk, with its name in `SourceResolution.would_create`. It checks the nearest existing ancestor with `os.access(W_OK | X_OK)` before reporting creation; strict stat/access failures refuse without mkdir. `_retain_prior_default_sources` uses `dataclasses.replace` to preserve that field. After consumer-boundary filtering, push preview refuses if the recovered prior still lists files for that root; otherwise the root is omitted from the source list passed to the unchanged deletion proof. A root present with `events/` gone still truthfully previews deletions; real-push bootstrap policy is deferred (E4).
