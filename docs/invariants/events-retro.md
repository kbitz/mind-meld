...
2. **`dry_run` no-op (preview contract).** `mm push --dry-run` must not mutate **snapshot publication**: no blob, manifest, last_seen, or event-file writes. The events tail returns immediately when `dry_run=True`, mirroring `_ensure_device_registered`'s same gate (codex review 2026-04-25). Strict scan and deletion-proof failures still refuse. The command's **setup** path can still prompt config migration, persist a missing crypto fingerprint, or bootstrap the mm-events source directory; do not weaken this invariant to match that pre-existing setup defect, and do not describe the whole current command as globally read-only. The setup repair is a separate follow-up.
