...
### [ship:severity=minor] Stale Track/version literals in code and test comments (15 across 9 files)
- **Description:** the 2026-09-03 renumber (Groups 45-50) fixed every stale cross-reference in docs/, but the same epoch-rot lives in code and test comments a docs-only PR does not touch. Fix the batch in the next code-touching Track. **Completeness criterion is the grep, not this list:** `grep -rn "Track 37A\|Track 37B\|Track 39A\|Track 42A" src tests` — every hit must either name a live card or carry dated lineage. (`Track 37A` hits in `tests/test_bin_check.py:1` and `tests/test_docs_routing.py:16`/`:666` are the CORRECT shipped verification-command Track — leave those.)
