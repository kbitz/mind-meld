...
_touches: src/mind_meld/manifest.py, src/mind_meld/cli.py, src/mind_meld/config.py, tests/test_manifest.py, tests/test_config.py, tests/test_pull_helpers.py_
...
- **Regression pins** -- three, one per behavior change. Symlinked `AGENTS.md` never enters the manifest (`test_manifest.py`, since it is a `walk_generic_source` assertion); a local symlink survives a pull that would have clobbered it (`test_pull_helpers.py`); and the narrowed codex/opencode source still includes hand-authored `AGENTS.md` while excluding the `bin/apply`-generated `skills/`, `commands/`, `agents/` trees (`test_config.py`). That third pin matters because `DEFAULT_SOURCES` is a routed invariant surface and task 3 currently has no coverage at all. _tests, ~90 lines._ (M)
