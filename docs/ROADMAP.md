...
_2 tasks . ~140 LOC . medium risk . 5 files_
...
_out: 39A_
_read-first: 36A, 28A, docs/invariants/sync.md, docs/invariants/events-retro.md (skill-link sections)_
_produces: mm stops syncing a host the user does not run, and stops installing a skill link for it_
_session: fresh · effort: medium · verify: pytest tests/test_config.py tests/test_skill_link.py tests/test_source_toggle.py tests/test_docs_routing.py; ruff check .; ruff format --check ._

_The user-visible half of the removal, split from 36A because it needs a migration and 36A does not._

- **Drop the source, keep existing configs loading** -- remove the `opencode` entry from `config.DEFAULT_SOURCES` and its half of `_GENERATED_HOST_SKILL_GLOBS` (the five globs are duplicated across the codex and opencode entries; only the opencode copy goes). **This needs a migration, not just a deletion:** an existing `config.toml` carries an enabled `[[sync.sources]] name = "opencode"` with `type = "generic"`, which keeps loading as a plain generic source after the default is gone — verified on device `889e42c0`, where it is enabled today. `mm migrate-config` must drop it, and the release note must name `mm disable-source opencode` for anyone who skips the migration. Removing it must not generate deletion tombstones for the peer's copies. _config.py, ~80 lines._ (M)
- **Drop the skill-link row** -- remove the `opencode` row from `skill_link.AGENT_ROWS`, which owns `~/.config/opencode/skills/retro-fleet` and its two markers. This is mm retiring a link **it** owns, which is NOT the user-deletion case Track 28A's guard protects — read that guard first so the two paths stay distinguishable, and leave an orphaned link on disk rather than growing a reaper for a directory mm does not own. _skill_link.py, ~60 lines._ (M)
