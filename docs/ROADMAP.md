...
Two hotfixes ship first — both are regressions on shipped behavior, not deferred scope. The correctness sweep then closes the seams v0.12.14 and v0.12.15 left behind, and the fleet model-mix work proceeds in parallel wherever its footprint is disjoint.
...
_5 tasks . ~140 LOC . medium-high risk . events.py hot path_
...
- **Regression pins** -- a jsonl with one bad byte does not kill the tail; a deadline-truncated walk raises `pre_skills_peers` instead of reporting zero. _tests/test_events.py, ~50 lines._ (S)
- **Invariant doc** -- document deadline exhaustion as the third `pre_skills_peers` population (the doc currently enumerates exactly two). _events-retro.md, ~15 lines._ (XS)
...
### Phase 3: /full-review correctness sweep

**End-state:** The seams that v0.12.14 and v0.12.15 left behind are closed, the test suite stops writing to the developer's real agent config dirs, and the invariant docs route to functions that still exist — so the fleet-model-mix work isn't built on sand.
**Groups:** 15, 16, 17, 18, 19

#### Group 15: retro-fleet skill installer correctness + test isolation

##### Track 15A: One gate predicate, an honest exit code, and a conftest fixture
_5 tasks . ~180 LOC . medium risk . cli.py + conftest_
_touches: src/mind_meld/cli.py, tests/conftest.py, tests/test_skill_link.py_

- **One existence predicate across all gates** -- the three self-heal gates use three pre-checks and none matches the installer, so installing Codex after mm is healthy for Claude never yields a link — permanently, not for 24h. Reproduced. Align on `agent_dir.exists()`, which `install_skills_cmd` already uses. _cli.py, ~40 lines._ (S)
- **Table-driven targets** -- replace six near-identical wrappers, six marker constants, and seven hardcoded path sites with one `_SKILL_TARGETS` tuple, so a fourth agent is a row not three functions. _cli.py, ~60 lines reducible._ (S)
- **Third bucket + non-zero exit on partial install** -- a target whose `symlink_to` raised lands in neither `installed` nor `conflicts` and one success returns 0; pre-v0.12.14 the single-target form always hit `Exit(1)`. Add `not_installed`, flatten the four-conditional tail. _cli.py, ~35 lines._ (S)
- **`_isolate_skill_links` autouse fixture** -- the installer now mkdirs and symlinks into three real user dirs and `_config_dir()` hardcodes `~/.config/mind-meld`; `pytest` on a dev Mac mutates the developer's real `~/.codex` and `~/.config/opencode`. Consider deleting `_config_dir()` in favour of `config.CONFIG_DIR` so one setattr covers the markers. _tests/conftest.py, ~30 lines._ (S)
- **Composition test + `exist_ok` + per-target notices** -- pin the gate×installer composition `test_skill_link.py` currently misses; add `parents=True, exist_ok=True` to the one mkdir of 22 without it; interpolate `safe_str(str(target))` into the three failure notices that don't say which agent failed. _cli.py + tests, ~40 lines._ (S)

#### Group 16: `mm gc` dry-run contract + the missed `safe_str` sites

##### Track 16A: Make every reaper honest, and finish the sanitization sweep
_4 tasks . ~130 LOC . low risk . cli.py_
_touches: src/mind_meld/cli.py, tests/test_conflict_copy.py_

- **Collapse the two conflict/event reapers onto one loop** -- `_gc_old_conflict_files` only increments `reaped` inside `if not dry_run:`, so `--conflicts --dry-run` always reports "would reap 0"; its ~90%-identical mirror has the `else` branch. Collapse rather than patch the increment so the next divergence can't happen. _cli.py, ~50 lines reducible._ (S)
- **`_gc_token_cache` honors `--dry-run`** -- it prints "dry-run; skipping" and reports nothing while every sibling prints `would delete (age Nd): <path>`; its own comment promises the opposite. The reap predicate is pure, so a read-only pass is cheap. _cli.py, ~30 lines._ (S)
...
#### Group 17: cli.py duplication sweep

##### Track 17A: Retire the copy-paste that reviews keep re-finding
_5 tasks . ~195 LOC reducible . low-medium risk . cli.py_
_touches: src/mind_meld/cli.py, src/mind_meld/conflictdiff.py_

- **Interactive counterpart of `_auto_command_setup`** -- four commands repeat the config/passphrase/lock preamble; the auto pair was migrated, the interactive half never was. _cli.py, ~60 lines reducible._ (M)
- **Move diff colouring into `conflictdiff.render_diff_lines(diff, cap)`** -- the loop is duplicated verbatim across both prompt sites with silently drifted caps (60 vs 80). This is the leaf-rendering shape the module exists for; the site-level dispatch CLAUDE.md protects is the choice logic, not the colouring. _conflictdiff.py + cli.py, ~40 lines._ (S)
- **Resolve the `b`/`both` alias** -- copy-pasted into both prompts, both comments say "removed at 1.0". Decide: delete now, or one `_normalize_conflict_choice`. _cli.py, ~20 lines._ (XS)
- **Three small collapses** -- `_empty_outcomes()` at its only caller; the crypto lost-race branch actually falling through as its comment claims; drop the `else` that wraps 40 lines after a `continue`. _cli.py, ~30 lines._ (S)
- **Two tail/preamble helpers** -- `enable_source`/`disable_source` share a 7-line preamble; the `autopull`/`autopush` except-tails differ by one verb and one breadcrumb label (verify `test_silent_failure_contract.py` doesn't pin them separately). Also materialise `_push_core`'s diff list once instead of walking `iter_source_diffs` twice per push. _cli.py, ~45 lines._ (S)

#### Group 18: Dead constants, stale shims, stray function-local imports

Pure deletion. Nothing here is additive.

##### Track 18A: One subtraction pass across seven modules
_5 tasks . ~190 LOC (mostly removals) . low risk . multi-module_
_touches: src/mind_meld/cli.py, src/mind_meld/token_usage.py, src/mind_meld/events.py, src/mind_meld/skills/retro_fleet/aggregator.py, src/mind_meld/pullhistory.py, src/mind_meld/seen_sources.py, src/mind_meld/upgrade.py, tests/test_token_usage.py_

- **Thirteen function-local re-imports in cli.py** -- nine shadowing module-scope `hashlib`/`json`/`secrets`/`datetime`, four re-importing the conflict-filename helpers. Ruff F811 cannot see function-local shadowing, so they survive lint indefinitely. Also rename the `sidecar` loop variable that shadows the module. _cli.py, ~30 lines._ (S)
- **Three dead constants** -- `pullhistory.HISTORY_PATH` and `seen_sources.SEEN_PATH` have zero references and sit beside the call-time resolvers the isolation fixtures depend on (a future caller picking the frozen one breaks them); `upgrade.py` carries a dead `fsutil` import plus a `_ = fsutil` keeper plus a comment explaining the dead code. _3 modules, ~25 lines._ (S)
- **token_usage: `CacheEntry` and the shim-over-a-shim** -- `CacheEntry` has zero references AND misdescribes the shape (no `offset`/`head`/`head_len`/`tail_msg_ids`), leaving `_resume_plan`'s isinstance gauntlet as the only documentation. Delete or correct it, and delete `walk_jsonl_token_buckets` (every caller is a test). _token_usage.py + tests, ~60 lines._ (S)
- **events.py: dead except members + the twice-written collector** -- both narrow types in `except (CancelledError, FuturesTimeoutError, Exception)` are `Exception` subclasses; the future-result block is written twice with drifting except clauses. _events.py, ~40 lines._ (S)
- **aggregator.py: hoist eight function-local imports** -- three import overlapping names; `_import_canonicalize`'s "tests can run without mind_meld" rationale died when the module started importing `mind_meld.identity` at top level. Keep only a genuine cycle-breaker, with a comment. _aggregator.py, ~35 lines._ (S)

#### Group 19: Docs drift + invariant re-anchor

_Depends on: Group 13_

##### Track 19A: Make the routing table land on code that exists
_4 tasks . ~120 LOC . low risk . docs only_
...
- **Regenerate the skill-installer invariant section** -- it still describes one target, two markers, one check-due function, and a "skills-dir-absent → silent skip" branch that v0.12.14 replaced with dir creation; the "exits 1" claim no longer holds either. _events-retro.md, ~40 lines._ (S)
- **Re-anchor citations on function names, not line numbers** -- every pinned line has drifted (`aggregator.py:1862`→~1969, `830`→~814, `cli.py:2886`→3094, `5688`→7407, `1115`→1164, sync.md's `1581`→1834). The routing table already uses function names successfully; finish the job so the next refactor doesn't re-break it. _CLAUDE.md + invariants, ~40 lines._ (S)
- **Document the new sources and `fsutil`** -- nothing mentions the `codex`/`opencode` `DEFAULT_SOURCES` entries despite `config.py:DEFAULT_SOURCES` being a routed invariant surface; the Source Layout omits `fsutil.py`, the only module missing, despite it owning the flock-append/atomic-write conventions four modules route through. Add the pointer row for the post-split installer function names. _CLAUDE.md, ~25 lines._ (S)
...
#### Group 20: Card + pull-request totals from existing data
...
##### Track 20A: ASCII card MODELS block + subject pull-request parser
...
- **`host_family(model)` in aggregator** -- `claude-*` → claude; `gpt-*` / `o1` / `o3` / `o4-*` / `*codex*` → codex; `grok-*` → grok; else other. OpenCode-hosted `claude-*` counts as claude. Track 25A will switch this import to `host_usage` once that module exists. _aggregator.py, ~20 lines + table tests._ (S)
...
- **GitAggregate fields** -- `prs: int`, `prs_by_host: dict[str, int]` (all `unknown` until Track 26A). Snapshot `metrics` grows `prs` additively. _aggregator.py, ~20 lines._ (XS)

#### Group 21: Codex walker + isolated host-token cache

_Depends on: none_

First half of the old Track 14A, which the audit flagged at 450 LOC. New module, new cache file. Must not touch `session-tokens.json` (`is_cache_cold` is global and load-bearing).

##### Track 21A: `host_usage.py` foundation + Codex walker
...
- **`host_family` + DayBucket mapping** -- same classifier as 20A, owned here as the long-run source of truth. Map each host's usage fields onto `TOKEN_FIELDS` (`input` / `cache_create` / `cache_read` / `output`). Reasoning tokens fold into `output`, documented. _host_usage.py, ~40 lines._ (S)
...
- **Isolated cache** -- `~/.config/mind-meld/host-tokens.json` via `lockedjson`. Deadline gate at the start of each session file. Cold+autopush skip is a CLI policy (Track 24A), not this module. _host_usage.py + tests, ~50 lines._ (S)

#### Group 22: Grok Build + OpenCode walkers

Second half of the old 14A. Split out rather than merged because each parser has substantial unique surface and its own fixture.

##### Track 22A: Grok Build and OpenCode session parsers
...
#### Group 23: Events-tail / backfill consolidation

_Depends on: Group 18_

Lands before the host-usage wire so the new walk is grafted once, not onto both copies.

##### Track 23A: `_capture_events_snapshot` + tail integration polish
_4 tasks . ~150 LOC . medium risk . src/mind_meld/cli.py_
_touches: src/mind_meld/cli.py_

- **Extract `_capture_events_snapshot(...)` helper** -- pull out the 90% shared structure between `_run_events_tail` and `_run_events_backfill` (gate, deadline math, claude_paths walk, agg_projects, s_rows). The in-code comment admits the deadline-refresh bug was fixed twice, once per copy. _cli.py, ~80 lines reducible._ (M)
...
- **Budget the root discovery, and stop paying for it twice** -- `discover_git_roots` runs with no wall-clock budget (~107 serial `git rev-parse` spawns on the measured Mac), is invisible to the budget notice because `deadline` is reset after it, and runs a second time from `identity._gather_per_repo_emails` on a cold cache. Memoize per-process first — that alone halves the cold path. Also delete the dead `deadline` assignment at the head of both walkers, and surface or drop `warm_token_cache_inline`'s discarded `(walked, skipped)` counts. _cli.py, ~50 lines._ (S)
- **Substantive-change gate timing** -- gate sees the pre-tail manifest; on UTC midnight rollover with zero source changes, no daily mm-push row lands. Verify whether monitoring depends on a daily heartbeat row; either lift the gate when the cursor is >24h stale OR document that no-op pushes don't advance it. _cli.py, investigative._ (S)

#### Group 24: `host-usage-snapshot` on the wire

_Depends on: Group 22, Group 23_
...
##### Track 24A: Emit host-usage-snapshot from tail and backfill
...
- **Tail / backfill policy** -- after the Claude `walk_done` snapshot (do not move it — v0.12.9), reset a fresh host deadline. Mirror `_decide_token_walk_policy` against `host-tokens.json`. Cold+autopush → no row. `dry_run` → no-op. Forensic `try/except`. Init backfill writes the snapshot (still no mm-push row). _cli.py, ~80 lines._ (M)
...
- **D4 pins** -- skip does not emit; warm-then-cold does not write an empty row that would latest-wins-wipe a prior snapshot. Aggregator fallback to the last present snapshot per device is Track 25A, but the emit side must not create the wipe. _tests/test_events.py, ~40 lines._ (S)
...
#### Group 25: Aggregator host-token merge

_Depends on: Group 20, Group 24_

First half of the old Track 17A, which the audit flagged at 350 LOC. Lights up Codex / Grok card rows after one fleet push. Claude cost line stays isolated so unpriced GPT/Grok volume does not flip it to `>=`.

##### Track 25A: Latest snapshot per device, sibling token map, mixed-fleet breadcrumb
...
#### Group 26: Pull-request attribution + card fill-in

Second half of the old 17A.

##### Track 26A: Attribution heuristic, host rows, snapshot metrics
...
Group 15: Skill installer correctness + test isolation
  +-- Track 15A ........... ~M . 5 tasks

Group 16: mm gc dry-run contract + safe_str sites
  +-- Track 16A ........... ~S . 4 tasks

Group 17: cli.py duplication sweep
  +-- Track 17A ........... ~M . 5 tasks

Group 18: Dead constants + stale shims
  +-- Track 18A ........... ~M . 5 tasks

Group 19: Docs drift + invariant re-anchor
  +-- Track 19A ........... ~S . 4 tasks

Group 20: Card + pull-request totals
  +-- Track 20A ........... ~M . 4 tasks

Group 21: Codex walker + isolated cache
  +-- Track 21A ........... ~M . 3 tasks

Group 22: Grok + OpenCode walkers
  +-- Track 22A ........... ~M . 2 tasks

Group 23: Events-tail / backfill consolidation
  +-- Track 23A ........... ~M . 4 tasks

Group 24: host-usage-snapshot wire
  +-- Track 24A ........... ~M . 5 tasks

Group 25: Aggregator host merge
  +-- Track 25A ........... ~S . 3 tasks

Group 26: Pull-request attribution + card
  +-- Track 26A ........... ~S . 2 tasks
```

**Total: 2 phases . 14 groups . 14 tracks remaining.**
