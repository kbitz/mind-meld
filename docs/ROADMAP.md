...
- **Drop redundant v1 `"files"` key in v2 manifests** — `manifest.py:357-397`, `cli.py:566-571`: `build_manifest_v2` and `_merge_manifests` both write a top-level `"files"` key that no downstream reader consumes; `normalize_manifest`'s v1→v2 promotion is the only real compat shim. (Note: `cli.py:566-571` change also belongs logically here, but touches cli.py — land it as part of Track 1A instead if Group 1 ordering is preserved.) _src/mind_meld/manifest.py, ~20 lines._ (S)
