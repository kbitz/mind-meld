...
`safety.py` (v0.11.1, extended v0.14.4) — peer-controlled string sanitization (`safe_str`, `safe_text`, `strip_terminal_escapes`, `safe_terminal_str`); cli.py re-exports `safe_str` / `safe_text` / `strip_terminal_escapes` for backwards compat. New tests should import from `mind_meld.safety` directly. Use `safe_terminal_str` for single-line plain stderr; `safe_str` is Rich markup only and is not a no-control guarantee. See `docs/invariants/init-devices.md`.
...
| `host_usage.py:_cached_last_reason` / (the Grok cache-root `last_reason` field) / `events_tail.py:_HOST_PERMANENT_REASONS` / `cli.py:status` / `diag` / (their permanent-reason branches) | `docs/invariants/events-retro.md` |
...
| `token_usage.py:PRICING` / `MODEL_FAMILY_TIERS` / `PRICING_FAMILY_BY_MODEL` / `VENDOR_FAMILY_TIERS` / `resolve_prices` / `model_family` / `estimate_cost` / `_CACHE_WRITE_MULT` | `docs/invariants/events-retro.md` (cost-estimation section) |
