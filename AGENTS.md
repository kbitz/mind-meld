...
| `host_usage.py:_carry_reason` / `_cached_last_reason` / `_cached_reason_since` / (both cache roots' last_reason and last_reason_since fields) / `cli.py:status` / `diag` / `_host_usage_blocker` | `docs/invariants/events-retro.md` (standing read blockers) |
