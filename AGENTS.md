...
Push flags: `--capture-usage` (61A, corrected 64A) refreshes host usage on an attended push. Shares `_capture_host_snapshot` with tail/backfill; always suppresses tail host capture. Usage-only refreshes skip Git/session/mm-push capture and preserve push counts/cursor; user-source byte, mtime or selection changes run the ordinary activity tail and count once. Exit 0 requires the row in the accepted manifest; 4 means row written but its revision is not in the accepted manifest while content sync was otherwise fine; 1 means no row written, append failed, or push stopped before acceptance (content not pushed); 2 incompatible `--dry-run`. Status/diag share `latest_event_rows`, fleet acceptance/order and pure five-state readiness, with `host_publication` separated from cache state. See `docs/invariants/events-retro.md`.
...
|---|---|
| `config.py:UsageCaptureReadiness` / `usage_capture_readiness` / `usage_capture_remedy` / `cli.py:_reader_capture_readiness` / `_usage_capture_needs_upgrade` / `_usage_capture_remedy` / `_print_usage_push_mode` / `_push_result_or_none` / `PushResult.content_changed` / `PushResult.content_files` / `events.py:GIT_SNAPSHOT_ORIGIN_INIT` / `write_push_event` (batch origin guard) / `events_tail.py:_capture_event_snapshots` (`origin`) / `_run_events_tail` (`capture_activity`) / `cli.py:_push_core` (`usage_capture`, content gate) | `docs/invariants/events-retro.md` and `docs/invariants/sync.md` |
...
| `aggregator.py:_render_token_block` / `_unpriced_token_summary` / `_short_model_name` / `_format_usd` / `_long_context_cause` | `docs/invariants/events-retro.md` (cost-estimation section) |
