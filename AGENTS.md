...
- **Scoped sync.** Built-in sources are allowlisted: Claude Code syncs its `memory/` and `todos/` project data; Codex syncs its documented customizations; Grok syncs only hardcoded `skills/`, `commands/`, and `rules/`. An `opencode` source remains in the default config until it is retired. Session databases, credentials, and whole-file settings that may contain credentials stay local.
