...
| `events.py:make_host_usage_snapshot` (`partial_sources` / `degraded_sources` disjointness) / `events_tail.py:HostUsageCapture` / `_merge_partial_days` / `_canonical_partial` / `_merge_warm_retry_capture` | `docs/invariants/events-retro.md` (coverage states) |
