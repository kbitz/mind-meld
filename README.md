...
| `mm diag` | Dump non-secret crypto, sync, breadcrumb, `retro-fleet` skill-link state, Grok `host_skill_discovery`, and git-root `discovery` for triage. Runs without a passphrase, and without a valid config. `--json` for machine-readable output. Top-level keys: `mm_version`, `config`, `crypto_init`, `root_salt_drift`, `sidecar`, `storage_inventory`, `last_autorun`, `skill_links`, `host_skill_discovery`, `discovery`. Each `skill_links` row includes `maintain_links` (`enabled` / `disabled (…)` / `unknown (config invalid: …)` / `unknown (policy not resolved)`). `host_skill_discovery` is not a skill-link row. |
...
**This discovery fix is not retroactive.** The retro cursor already advanced past every lost interval and there is no backfill verb, so earlier windows stay incomplete. Every Mac must upgrade individually, because discovery happens locally.
...
**`mm status` shows a `degraded` breadcrumb.** The sync itself succeeded; the `detail` field names which optional signal was lost. Fleet-retro capture and host-usage snapshots are best-effort and never block content sync. If the detail mentions git repository discovery, run `mm diag` — a later push does not recapture the omitted interval.

**`mm retro-fleet` under-counts commits, or `mm diag` shows `status: empty` / `exceeded`.** Discovery is local to each Mac. Upgrade that machine, then `mm push`. Capture is fixed going forward; earlier windows stay incomplete. To force a machine to include a repo Claude Code has no session for, add it to `[retro] repo_roots` (absolute paths) and verify with `mm diag`.
